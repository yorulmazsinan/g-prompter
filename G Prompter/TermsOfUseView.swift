...
struct TermsOfUseView: View {
    ...
    @State private var termsOfUse: String = "Yükleniyor..."
    @State private var isLoading: Bool = true // Yükleme durumu
    @State private var hasError: Bool = false // Hata durumu
    
    private let url = URL(string: "https://raw.githubusercontent.com/yorulmazsinan/g-prompter/refs/heads/main/terms-of-use.md")!
    ...
    func fetchTermsOfUse() {
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            guard error == nil else {
                print("Ağ hatası: \(error!.localizedDescription)")
                DispatchQueue.main.async {
                    self.termsOfUse = "Kullanım koşulları yüklenemedi."
            ...
            if let data = data, let text = String(data: data, encoding: .utf8) {
                DispatchQueue.main.async {
                    self.termsOfUse = text
                    self.isLoading = false
                }
            } else {
                DispatchQueue.main.async {
                    self.termsOfUse = "Geçersiz veri alındı."
                    self.isLoading = false
                }
            }
        }
